...
def is_product_continuously_offered(produkt_id, haendler_bez, seal_date_str, angebot_data, weeks):
    ...
    """
    logger.info(
        f"Starting check for product {produkt_id} offered by firm {haendler_bez} around the seal date {seal_date_str}."
    )

    if weeks < 4:
        logger.warning(
            "Weeks parameter is less than 4. Not enough weeks to determine continuous offer. Returning False."
        )
        return False
    ...
    offered_period_start = (
        seal_date_start_of_week - dt.timedelta(weeks=OFFER_TIME_SPELLS_PREPROCESSING_WEEKS_POST_SEAL_CONSIDERED)
    )
    offered_period_end = (
        seal_date_start_of_week + dt.timedelta(weeks=OFFER_TIME_SPELLS_PREPROCESSING_WEEKS_POST_SEAL_CONSIDERED + 1)
    )
    ...
    logger.info(
        f"Filtered data contains {len(angebot_data_filtered_haendler_produkt)} rows for product "
        f"{produkt_id} from firm {haendler_bez}."
    )
    ...
    for i in range(weeks * 2 + 1):
        ...
        if week_number not in offered_weeks:
            ...
            if weeks_with_no_offers > 1:
                logger.info(
                    "Product is not continuously offered. Exceeded the allowed number of weeks with no offers. "
                    "Returning False."
                )
                return False
...
def filter_continuously_offered_products(haendler_bez, top_products, seal_date_str, angebot_data, week_amount):
    ...
    angebot_data_filtered_haendler['dtimebegin'] = angebot_data_filtered_haendler['dtimebegin'].apply(
        lambda x: dt.datetime.fromtimestamp(x))
    angebot_data_filtered_haendler['dtimeend'] = angebot_data_filtered_haendler['dtimeend'].apply(
        lambda x: dt.datetime.fromtimestamp(x))

    for produkt_id in top_products:
        if is_product_continuously_offered(produkt_id, haendler_bez, seal_date_str, angebot_data_filtered_haendler,
                                           week_amount):
...
def get_rand_max_20_counterfactual_firms(product_id, seal_date_str, angebot_data, seal_firms, allowed_firms):
    ...
    """
    logger.info(
        f"Getting random max {RANDOM_COUNTERFACTUAL_FIRMS_AMOUNT} "
        f"counterfactual firms for product {product_id} and seal date {seal_date_str}."
    )
...
def get_offered_weeks(angebot_data, prod_id, firm_id, seal_date_str):
    ...
    for _, row in filtered_angebot.iterrows():
        ...
        current_date = start_date
        while current_date <= end_date:
            week_number = int(calculate_running_var_t_from_u(int(current_date.timestamp())))
            offered_weeks.add(week_number)
            current_date += dt.timedelta(days=1)
    ...
    offered_weeks = {
        ...
        if week >= int(
            calculate_running_var_t_from_u(
                seal_date - 26 * UNIX_WEEK)
        )
    }
