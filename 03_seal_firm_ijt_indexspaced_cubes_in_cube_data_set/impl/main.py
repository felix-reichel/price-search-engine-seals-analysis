...
def generate_months_around_seal(seal_year, seal_month):
    ...
    for i in range(-6, 7):  # from 6 months before to 6 months after
        new_date = datetime(seal_year, seal_month, 1) + relativedelta(months=i)
        year = new_date.year
...
def main(parallel=False):
    ...
    with Manager() as manager:
        ...
        seal_firm_data = [
            (
                haendler_bez,
                seal_change_firms.iloc[i, 2],  # Geizhals identifier
                seal_change_firms.iloc[i, 3],  # Seal change date
                seal_firms,
                products_df,
                retailers_df,
                allowed_firms,
